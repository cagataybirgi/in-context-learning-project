...
def run_experiment(
    ...
    *,
    ...
    for idx, sample in enumerate(tqdm(samples, desc=f"{dataset_name}/{strategy}", ncols=72)):
        ...
        # Augmented metrics.
        # substring_match: gold scalar appears verbatim in the extracted
        # prediction (or the raw generation as a fallback).
        # is_numeric_close: only meaningful for GSM8K.
        # token_f1 / rouge_l: full generation vs full gold text (for GSM8K
        # this is the gold reasoning chain; for StrategyQA the yes/no label).
    ...
    try:
        ...
        # Augmented-metric aggregates.  Guarded so older CSVs without the
