...
Adds the augmented metric columns — `substring_match`, `is_numeric_close`,
...
Caveat: historical CSVs only store the extracted scalar in `gold_answer`
(not the full GSM8K reasoning chain), so retroactive token_f1 / rouge_l
compare against that scalar.  Fresh runs from `runner.py` compare against the
full gold text, which is the more meaningful scope.
